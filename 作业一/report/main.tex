...
\begin{abstract}
This project focuses on developing and training a neural network for accurate classification of grayscale fashion items in the Fashion-MNIST dataset. The neural network architecture comprises three linear layers, and various components such as activation functions and loss functions are explored to optimize performance. The report details dataset downloading, architectural choices, training processes, and evaluation metrics, emphasizing the importance of meticulous design and optimization for effective neural network development in image classification tasks.
\end{abstract}
...
Having successfully trained the three-layer neural network and saved the weight parameters, we now proceed to load the trained model and evaluate its performance on the testing dataset. The accuracy table is presented in Table \ref{tab:accuracy-table}.
...
T-shirt/top & 0.847 & 0.836957 & 0.841948 \\ \hline
Trouser & \textcolor{red}{0.973} & 0.988821 & 0.980847 \\ \hline
Pullover & 0.812 & 0.817724  & 0.814852 \\ \hline
Dress & 0.879 & 0.904321 & 0.891481 \\ \hline
Coat & 0.83 & 0.793499 & 0.811339 \\ \hline
Sandal & 0.966 & 0.974773  & 0.970367 \\ \hline
Shirt & \textcolor{blue}{0.719} & 0.72334 & 0.721163 \\ \hline
Sneaker & 0.959 & 0.946693 & 0.952807 \\ \hline
Bag & 0.97 & 0.973896 & 0.971944 \\ \hline
Ankle boot & 0.962 & 0.962963 & 0.962481 \\ \hline
\textcolor{orange}{Total} & \textcolor{orange}{0.8917} & \textcolor{orange}{0.892299}  & \textcolor{orange}{0.891923} \\ \hline
\end{tabular}
\caption{Accuracy table}
\label{tab:accuracy-table}
\end{table}

The model achieves an accuracy of $89.17\%$ on the testing dataset. It performs best on the label Trouser with an accuracy exceeding $97\%$ and worst on the label Shirt with an accuracy below $72\%$.
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=0.4]{images/original_image.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=0.4]{images/first_layer.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=0.4]{images/second_layer.png}
\caption{Images after the second layer}
\label{fig:second-layer}
\end{figure}

Upon examination, we notice that the images after the first layer for each class don't exhibit much similarity. This may be because the neural network hasn't learned sufficient information about the dataset in one single layer. However, in the images after the second layer, we can clearly observe that the network has learned distinct patterns for each class. Notably, the Trouser class shares the most similarity between the selected 10 images, while the patterns for the Shirt class are less consistent, consistent with our observations in the accuracy table.
